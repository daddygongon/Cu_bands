...
#    labels = [r"$\Gamma$", r"$X$", r"$W$", r"$K$", r"$\Gamma$", r"$L$", r"$U$", r"$W$", r"$L$", r"$K,U$", r"$X$" ]
    labels = [r"$\Gamma$", r"$X$", r"$W$",  r"$L$", r"$\Gamma$", r"$K$", r"$X$"]
    # density of states
    dosrun = Vasprun("./dos_calc_fine/vasprun.xml")
    spd_dos = dosrun.complete_dos.get_spd_dos()

    # bands
    run = Vasprun("./band_calc_test/vasprun.xml", parse_projected_eigen=True)
    bands = run.get_band_structure("./band_calc_test/KPOINTS",
                                   line_mode=True,
